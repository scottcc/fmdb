...
@interface FMDBDatabase (PrivateStuff)
...
@implementation FMDBDatabase (FMDBDatabaseAdditions)
...
*/
- (FMDBResultSet*)getSchema {
    
    //result colums: type[STRING], name[STRING],tbl_name[STRING],rootpage[INTEGER],sql[STRING]
    FMDBResultSet *rs = [self executeQuery:@"SELECT type, name, tbl_name, rootpage, sql FROM (SELECT * FROM sqlite_master UNION ALL SELECT * FROM sqlite_temp_master) WHERE type != 'meta' AND name NOT LIKE 'sqlite_%' ORDER BY tbl_name, type DESC, name"];
...
*/
- (FMDBResultSet*)getTableSchema:(NSString*)tableName {
    
    //result colums: cid[INTEGER], name,type [STRING], notnull[INTEGER], dflt_value[],pk[INTEGER]
    FMDBResultSet *rs = [self executeQuery:[NSString stringWithFormat: @"pragma table_info('%@')", tableName]];
...
- (BOOL)columnExists:(NSString*)columnName inTableWithName:(NSString*)tableName {
    ...
    //If this is not done FMDBDatabase instance stays out of pool
    [rs close];
